	...
	vector<vec3>& tangents, vector<vec3>& bitangents)
{
	for (unsigned int i = 0; i < vertices.size(); i += 3)
	{
		...
		vec3 v0 = vertices[i];
		vec3 v1 = vertices[static_cast<unsigned __int64>(i) + 1];
		vec3 v2 = vertices[static_cast<unsigned __int64>(i) + 2];

		vec2 uv0 = uvs[i];
		vec2 uv1 = uvs[static_cast<unsigned __int64>(i) + 1];
		vec2 uv2 = uvs[static_cast<unsigned __int64>(i) + 2];
...
int main(void)
{
	...
	GLuint depthProgramID = LoadShaders("shaders/depth_shader/depth.vertex",
										"shaders/depth_shader/depth.fragment"); //рендер сцены для источника света
	...
	bool sun = GetOBJ("obj/sun.obj", sun_v, sun_u, sun_n);
	...
	GLuint programID = LoadShaders("shaders/main_shader/main.vertex", "shaders/main_shader/main.fragment");

	GLuint programID1 = LoadShaders("shaders/cooktor_shader/cooktor.vertex", "shaders/cooktor_shader/cooktor.fragment");

	GLuint programID2 = LoadShaders("shaders/normalmapping_shader/normalmapping.vertex", "shaders/normalmapping_shader/normalmapping.fragment");

	GLuint sun_programID = LoadShaders("shaders/sun_shader/sun.vertex", "shaders/sun_shader/sun.fragment");
	...
	GLuint DiffuseTexture = GetBMPPicture("bmp/color.bmp");
	GLuint NormalTexture = GetBMPPicture("bmp/normal.bmp");
	...
	mat4 Projection = perspective(radians(45.0f), 4.0f / 3.0f, 0.1f, 100.0f);//матрица перспективной проекции
